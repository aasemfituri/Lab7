...
window.addEventListener('hashchange', () => {
  ...
  if(urlHash === '#home') {
    ...
    body.className = 'settings';
    title.innerHTML = 'Settings'; 
    history.replaceState({page: 1},'','#settings');
  } else {
    let num2 ='';
    for(let i = 6; i < urlHash.length; i++)
    {
      num2 += urlHash[i];
    }

    if(entryList[num2 - 1] === undefined)
    {
      ...
      body.className = 'single-entry';
      document.querySelector('entry-page').entry = entryList[num2 - 1];
      document.querySelector('header h1').innerHTML = 'Entry ' + num2;
      history.replaceState(entryList[num2-1],'','#entry' + num2);
    }
  }
});
